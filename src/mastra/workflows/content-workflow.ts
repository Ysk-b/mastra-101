...
// createStepを使用してステップを定義。
// 各ステップはID, 概要、入力スキーマ、出力スキーマ、実行関数を持っており、それぞれが独立した処理単位となる。
// これらのステップを組み合わせると、より複雑なワークフローを構築できる。
...
const aiAnalysisStep = createStep({
  ...
  // 各ワークフローのexecute関数では、第二引数にmastraインスタンスを受け取れる。
  // このmastraインスタンスを通じて、エージェントやツール、ストレージなどにアクセス可能。
...
// このワークフローは、入力内容がスキーマの条件を満たし、その後出力スキーマに従って結果を返す。
// 出力内容としては、thenに記載のassessContentStepの出力に基づく。出力内容はassessContentStepの出力に依存する。
// branchで
  .then(assessContentStep)
  .branch([
    // Branch 1: Short and simple content
    // assessContentStepの出力に基づき、渡されたinputDataの評価が条件を満たす場合にquickProcessingStepを実行
    [
      async ({ inputData }) =>
